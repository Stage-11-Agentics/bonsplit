...
final class BonsplitTests: XCTestCase {
    ...
    func testChromeActiveIndicatorColorUsesAppearanceOverride() throws {
        let appearance = BonsplitConfiguration.Appearance(
            chromeColors: .init(activeIndicatorHex: "#12AB34")
        )

        let color = try XCTUnwrap(TabBarColors.nsColorActiveIndicator(for: appearance).usingColorSpace(.sRGB))
